function [Lr, Sp, G1, G2, U, S, V,Eval] = gsp_fastmc_2g_online_knn2(X, M, gamma1, gamma2, Ninit, batch, iters, G1, G2, param)
...
Eval = [Eval norm(M.*(Lr-X),'fro')+gamma1*sum(gsp_norm_tik(G1new,Lr)) + gamma2*sum(gsp_norm_tik(G2new,Lr'))];
Sp = X - Lr;
